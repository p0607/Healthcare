/** Maps marketing service ids (frontend serviceSections) → booking serviceType for caregivers. */
...
function inferCaregiverCategory(selectedServiceIds) {
  const types = selectedServiceIds
    .map((id) => SERVICE_ID_TO_TYPE[id])
    .filter(Boolean);
...
module.exports = { SERVICE_ID_TO_TYPE, inferCaregiverCategory };
